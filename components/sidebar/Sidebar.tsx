...
export function Sidebar() {
    ...
    return (
        <aside className="flex flex-col w-[220px] shrink-0 h-full bg-gray-50 dark:bg-[#111113] border-r border-gray-200 dark:border-white/[0.07] overflow-hidden transition-all duration-200">
                    ...
                    return (
                        ...
                        >
                            ...
                            {section === "Pinned" && pinnedNodeData.length > 0 && (
                                <Badge className="ml-auto h-4 min-w-4 px-1 text-[9px] bg-violet-600/30 text-violet-300 border-violet-600/40">
            ...
            {/* Clear filters bar */}
            {localQuery.trim() && (
                <div className="flex items-center justify-between px-3 py-1.5 border-b border-gray-200 dark:border-white/[0.07] bg-violet-50 dark:bg-violet-900/10">
                    ...
                    <Button
                        ...
                        className="size-5 text-violet-600 dark:text-violet-400 hover:text-violet-800 dark:hover:text-violet-200"
                        title="Clear filters"
                    >
                        <X className="size-3" />
                    </Button>
                    ...
                    {activeSection === "Nodes" && (
                        <>
                            ...
                            {filteredNodes.length === 0 && (
                                <p className="text-xs text-zinc-500 dark:text-zinc-600 px-2 py-4 text-center">
                                    No nodes found
                                </p>
                            )}
                        </>
                            ...
                            {allTags.map((tag) => (
                                <button
                                    ...
                                    className={`px-2 py-0.5 rounded text-[10px] border transition-colors ${activeTag === tag
                                        ...
                                        : "bg-violet-600/20 text-violet-600 dark:text-violet-300 border-violet-600/30 hover:bg-violet-600/30"
                                        }`}
                                >
                                    {tag}
                                </button>
    ...
    return (
        <button
            ...
            className={`flex flex-col gap-0.5 w-full px-2.5 py-2 rounded-md text-left transition-colors ${isSelected
                ? "bg-violet-600/20 border border-violet-600/40 text-violet-700 dark:text-violet-200"
        ...
        >
            ...
            <div className="flex gap-1 flex-wrap">
                {node.tags?.slice(0, 2).map((tag) => (
                    <span
                        key={tag}
                        className="text-[9px] px-1.5 py-0 rounded bg-violet-600/20 text-violet-600 dark:text-violet-400"
                    >
                        {tag}
                    </span>
                ))}
            ...
            <div className="flex flex-wrap gap-1">
                {options.map((o) => (
                    <button
                        key={o}
                        onClick={() => onSelect(o)}
                        className="px-2 py-0.5 rounded text-[10px] bg-gray-100 dark:bg-white/[0.05] text-zinc-600 dark:text-zinc-400 hover:bg-violet-600/20 hover:text-violet-600 dark:hover:text-violet-300 border border-gray-200 dark:border-white/[0.06] transition-colors"
                    >
                        {o}
                    </button>
                ))}
